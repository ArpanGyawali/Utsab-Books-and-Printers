...
export default function PrintQuoteForm() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const pages = normalizeDigits(String(data.get("pages") ?? "").trim()) || "-";
    const color =
      data.get("color") === "color" ? t("colorColor") : t("colorBW");
    ...
    window.open(
      waLink(t("waTemplate", { name, phone, description, pages, color, binding })),
      "_blank",
      "noopener"
    );
  }
  ...
  return (
    <section className="mt-10 rounded-md border border-[var(--ink-faint)] bg-paper-shade/60 p-5 sm:p-6">
      ...
      <form onSubmit={onSubmit} noValidate className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <label className="block">
          <span className="mb-1 block text-sm font-medium">{t("pages")}</span>
          <input name="pages" inputMode="numeric" className={inputCls} />
        </label>
        <fieldset>
          <legend className="mb-1 text-sm font-medium">{t("color")}</legend>
          <div className="flex gap-4 pt-1.5">
            <label className="flex min-h-11 items-center gap-2">
              <input type="radio" name="color" value="bw" defaultChecked className="accent-[var(--accent)]" />
              {t("colorBW")}
            </label>
            <label className="flex min-h-11 items-center gap-2">
              <input type="radio" name="color" value="color" className="accent-[var(--accent)]" />
              {t("colorColor")}
            </label>
          </div>
        </fieldset>
